...
appRoutes.get("/today-is", (request, response) => {
  const dt1 = new Date();
  const dt2 = new Intl.DateTimeFormat("es-ES", {
    dateStyle: "full",
  }).format(dt1);
  response.json({
    hoyes: dt2,
...
appRoutes.post("/send-email", async (req, res) => {
  const {
    email: euser,
    asunto,
    mensaje,
  ...
  let emails = null;
  await axios
    .get(`http://localhost:3001/api/entities_f/${banco}`)
    .then((res) => {
...
appRoutes.post("/veirify-email", (request, response) => {
  let sql = "SELECT id";
  sql += " FROM referidos";
  sql += " WHERE email=?";
...
appRoutes.post("/user", async (request, response) => {
  let sql =
    "INSERT INTO referidos (id_ref,email,nombre,apellidos,celular,hash)";
  sql += " value (?,?,?,?,?,?)";
